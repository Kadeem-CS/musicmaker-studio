    ...
    <div className="flex flex-col h-[70vh] gap-6 animate-in fade-in duration-500">
      ...
      <Tabs defaultValue="compositions" className="flex-1 flex flex-col overflow-hidden">
        <TabsList className="grid w-full grid-cols-2 bg-slate-950/50 p-1 mb-4 border border-slate-800">
          <TabsTrigger value="compositions" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white font-bold">Saved Projects</TabsTrigger>
          <TabsTrigger value="uploads" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white font-bold">Audio Uploads</TabsTrigger>
        </TabsList>

        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <TabsContent value="compositions" className="space-y-3 mt-0">
            {compositions.length === 0 ? (
              <p className="text-center text-slate-600 italic py-10">No saved projects found.</p>
            ) : (
              compositions.map((comp) => (
                <Card key={comp.id} className="p-4 bg-slate-900 border-slate-800 hover:border-purple-500/40 transition-colors">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-white font-bold">{comp.name}</h4>
                      <p className="text-[10px] text-slate-500 font-mono">{comp.tempo} BPM</p>
                    </div>
                    <Button 
                      size="sm" 
          ...
          <TabsContent value="uploads" className="space-y-3 mt-0">
            {uploadedTracks.length === 0 ? (
              <p className="text-center text-slate-600 italic py-10">No audio files uploaded to server.</p>
            ) : (
